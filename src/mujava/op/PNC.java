...
public class PNC extends mujava.op.util.Mutator {
	public PNC(FileEnvironment file_env, ClassDeclaration cdecl,
	 ...
	 * @param child
	 */
	public void generateMutants(OJClass type, AllocationExpression p,
			Vector child) {
		// Does not care types of argument
		for (int i = 0; i < child.size(); i++) {
			InheritanceINFO info = (InheritanceINFO) child.get(i);
			// Examine equivalent -- by examine if child class has overriding
			// method.
			if (hasOverridingMethod(type, info)) {
				AllocationExpression mutant = (AllocationExpression) p
						.makeRecursiveCopy();
	...
	boolean isOverridingMethod(OJMethod parent, Method child) {
		...
		if (!(parent.getName().equals(child.getName())))
			return false;
		if (!(parent.getReturnType().getName().equals(child.getReturnType()
				.getName())))
			return false;
	...
	public void visit(AllocationExpression p) throws ParseTreeException {
		...
		if (MutationSystem.isPrimitive(type)) {
			...
			InheritanceINFO inf = MutationSystem
					.getInheritanceInfo(original_name);
			if (inf != null)
	 ...
	 * @param mutant
	 */
	public void outputToFile(AllocationExpression original,
	 ...
	 * pncPossible(String p, String c,ExpressionList arg_list){ try{ OJClass
	 * clazz; OJClass parent_class; clazz = OJClass.forName(c);
	 ...
	 * if(clazz.getDeclaredConstructor(types)!=null) return true; }else{ //
	 ...
	 * return true; } } clazz = parent_class; } return false; }catch(Exception
	 * e){ return false; }catch(Error er){ return false; } }
	 */
