...
public class OAN extends mujava.op.util.Mutator {
	...
	public OAN(FileEnvironment file_env, ClassDeclaration cdecl,
	 ...
	 * determine if the method is an overloading method that can accept the new
	 * list of argument, generate OAN mutants.
	 */
			...
			if (oM.length > 1) {
				...
				if (index >= 0) {
					for (i = 0; i < oM.length; i++) {
						if (i != index
								&& oM_helper.sameReturnType(oM[index], oM[i])
								&& oM_helper.compatibleParameter(oM[index],
										oM[i])) {
