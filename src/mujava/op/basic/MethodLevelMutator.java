...
public class MethodLevelMutator extends mujava.op.util.Mutator {
	...
	public MethodLevelMutator(FileEnvironment file_env,
	...
	String getMethodSignature(MethodDeclaration p) {
		...
		for (int i = 0; i < pars.size(); i++) {
			...
			if (tempParameter.indexOf("<") >= 0
					&& tempParameter.indexOf(">") >= 0) {
				tempParameter = tempParameter.substring(0,
						tempParameter.indexOf("<"))
						+ tempParameter.substring(
								tempParameter.lastIndexOf(">") + 1,
								tempParameter.length());
				str += tempParameter;
			} else {
				str += tempParameter;
			}
	...
	public String getSourceName(mujava.op.util.Mutator clazz) {
		...
		String dir_name = MutationSystem.MUTANT_PATH + "/"
				+ currentMethodSignature + "/" + getClassName() + "_"
				+ this.num;
	...
	public String getSourceName(String op_name) {
		// make directory for the mutant
		String dir_name = MutationSystem.MUTANT_PATH + "/"
				+ currentMethodSignature + "/" + op_name + "_" + this.num;
		File f = new File(dir_name);
		f.mkdir();
