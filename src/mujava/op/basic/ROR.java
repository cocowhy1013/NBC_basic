...
public class ROR extends Arithmetic_OP {
	...
	private void primitiveRORMutantGen(BinaryExpression exp, int op) {
		 ...
		 * mutants are generated For details, see the paper
		 * "Better predicate testing" by Kaminski, Ammann, and Offutt at AST'11
		 * This part is currently experimental, which means, users will not see
		 * this part during the new release
		 */
		/*
		 * if (op == BinaryExpression.GREATER) { //mutant >= mutant =
		 * (BinaryExpression)(exp.makeRecursiveCopy());
		 * mutant.setOperator(BinaryExpression.GREATEREQUAL); outputToFile(exp,
		 * mutant);
		 * 
		 * //mutant != mutant = (BinaryExpression)(exp.makeRecursiveCopy());
		 * mutant.setOperator(BinaryExpression.NOTEQUAL); outputToFile(exp,
		 * mutant);
		 * 
		 ...
		 * 
		 * //mutant > mutant = (BinaryExpression)(exp.makeRecursiveCopy());
		 * mutant.setOperator(BinaryExpression.GREATER); outputToFile(exp,
		 * mutant);
		 * 
		 * //mutant == mutant = (BinaryExpression)(exp.makeRecursiveCopy());
		 * mutant.setOperator(BinaryExpression.EQUAL); outputToFile(exp,
		 * mutant); }
		 * 
		 ...
		 * 
		 * //mutant <= mutant = (BinaryExpression)(exp.makeRecursiveCopy());
		 * mutant.setOperator(BinaryExpression.LESSEQUAL); outputToFile(exp,
		 * mutant);
		 * 
		 * //mutant != mutant = (BinaryExpression)(exp.makeRecursiveCopy());
		 * mutant.setOperator(BinaryExpression.NOTEQUAL); outputToFile(exp,
		 ...
		 * }
		 * 
		 * if (op == BinaryExpression.LESSEQUAL) { //mutant true
		 * outputToFile(exp, Literal.makeLiteral(true));
		 * 
		 * //mutant < mutant = (BinaryExpression)(exp.makeRecursiveCopy());
		 ...
		 * mutant); }
		 * 
		 * if (op == BinaryExpression.NOTEQUAL) { //mutant false
		 * outputToFile(exp, Literal.makeLiteral(true));
		 * 
		 * //mutant < mutant = (BinaryExpression)(exp.makeRecursiveCopy());
		 * mutant.setOperator(BinaryExpression.LESS); outputToFile(exp, mutant);
