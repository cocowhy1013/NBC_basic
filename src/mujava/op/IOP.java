...
public class IOP extends mujava.op.util.Mutator implements IOP_Helper {
	...
	public IOP(FileEnvironment file_env, ClassDeclaration cdecl,
	...
	boolean isOverridingMethodCallWithSameName(MethodCall p) {
		...
		if (!(p.getName().equals(containing_method.getName())))
			return false;
		if (!compatibleParameters(p.getArguments(),
				containing_method.getParameters()))
			return false;
	...
	public void outputToFile(CompilationUnit comp_unit,
			StatementList stmt_list, int index, int mod) {
		if (comp_unit == null)
			return;
