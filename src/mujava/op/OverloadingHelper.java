...
public class OverloadingHelper {
	...
	public Vector locationForParameterType(OJMethod big_m, OJMethod small_m) {
		...
		for (i = 0; i < locInfo.size(); i++) {
			...
			genEachPermutation(par_info.position, n_list, r,
					par_info.combination_num, 0);
		}
		return locInfo;
	}
	...
	// nCr = n*(n-1)*(n-2) ... *(n-r+1)
	public void genEachPermutation(int[][] list, int[] n, int r, int num,
			int start) {
			...
			for (i = 0; i < n.length; i++) {
				...
				if (n.length == 2 && r == 2) {
					...
					if (repeat != 1) {
						...
						genEachPermutation(list, sub_n, r - 1, repeat, start
								+ i * repeat);
					}
				}
			}
	...
	public void genIndex(int[][] list, Vector parLoc, int num, int parIndex,
			int start) {
				...
				for (i = 0; i < par_info.combination_num; i++) {
					for (j = 0; j < repeat; j++) {
						for (k = 0; k < par_info.sub_position.size(); k++) {
							...
							original_value = (Integer) par_info.original_position
									.elementAt(temp);
							sub_value = (Integer) par_info.sub_position
									.elementAt(k);
							list[index][sub_value.intValue()] = original_value
									.intValue();
						}
						index++;
					}
					if (repeat != 1) {
						genIndex(list, parLoc, repeat, parIndex + 1, start + i
								* repeat);
					}
				}
			} catch (Exception e) {
