...
public class OMD extends DeclAnalyzer {
	...
	private boolean omdPossible(OJMethod m1, OJMethod m2) {
		...
		related_index = getCompatibleParameterIndex(m1.getParameterTypes(),
	...
	private int[] getCompatibleParameterIndex(OJClass[] smallP, OJClass[] bigP) {
		...
		for (int i = 0; i < smallP.length; i++) {
			results[i] = relatedDifferentIndex(smallP[i].getName(),
					bigP[i].getName());
			if (results[i] != 0)
				diff = true;
		}
	...
	public void outputToFile(CompilationUnit comp_unit,
			MethodDeclaration original, OJMethod mutant) {
		if (comp_unit == null)
			return;
	...
	public OMD(openjava.mop.Environment oj_param0,
