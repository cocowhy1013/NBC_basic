...
public class OMR extends DeclAnalyzer {
		...
		for (i = 0; i < m.length; i++) {
			...
			if (omNum[i] > 1) {
				...
				for (j = 0; j < omNum[i]; j++) {
					for (k = 0; k < omNum[i]; k++) {
						if (j != k
								&& oM_helper.sameReturnType(overloadingM[j],
										overloadingM[k])
								&& oM_helper.compatibleParameter(
										overloadingM[j], overloadingM[k])) {
							generateMutant(overloadingM[j], overloadingM[k],
									comp_unit);
						}
					}
				}
	 ...
	 * @param comp_unit
	 */
	public void generateMutant(OJMethod m1, OJMethod m2,
	...
	public OMR(openjava.mop.Environment oj_param0,
