...
public class TestExecuter {
	...
	public TestExecuter(String targetClassName) {
		...
		MutationSystem.EXCEPTION_MUTANT_PATH = MutationSystem.MUTANT_HOME + "/"
				+ targetClassName + "/" + MutationSystem.EM_DIR_NAME;
	...
	public TestResult runClassMutants() throws NoMutantException,
			NoMutantDirException {
	...
	public TestResult runExceptionMutants() throws NoMutantException,
			NoMutantDirException {
	...
	public TestResult runTraditionalMutants(String methodSignature)
		...
		if (methodSignature.equals("All method")) {
			try {
				...
				System.err
						.println("Error in update() in TraditioanlMutantsViewerPanel.java");
			}
		} else {
			MutationSystem.MUTANT_PATH = original_mutant_path + "/"
					+ methodSignature;
	...
	/**
	 * Commented Out, this method is not used anymore get the result of the test
	 * under the mutanted program
	 * 
	 * @deprecated
	 * @param mutant
	 * @param testcase
	 ...
	 */
	/*
	 * void runMutants(Object mutant,Method testcase) throws
	 * InterruptedException{ mutantRunning = true; try{ // testcase execution
	 * mutant_result = testcase.invoke(mutant_obj,null); }catch(Exception e){ //
	 * execption occurred -> abnormal execution mutant_result =
	 * e.getCause().getClass().getName()+" : " +e.getCause().getMessage(); }
	 * mutantRunning = false; synchronized(lockObject){ lockObject.notify(); }
	 ...
	 * InterruptedException{ wait(timeOut); }
	 */
	 ...
	 */
	private String[] getMutants(String methodSignature)
		...
		if (mutantDirectories == null || mutantDirectories.length == 0) {
			if (!methodSignature.equals(""))
				System.err
						.println(" No mutants have been generated for the method "
								+ methodSignature
								+ " of the class"
								+ MutationSystem.CLASS_NAME);
			else
				System.err
						.println(" No mutants have been generated for the class "
								+ MutationSystem.CLASS_NAME);
	...
	public void computeOriginalTestResults() {
		...
		try {
			...
			for (int k = 0; k < testCases.length; k++) {
				Annotation[] annotations = testCases[k]
			...
			for (int i = 0; i < tr.mutants.size(); i++) {
				...
				try {
					...
					Runnable r = new Runnable() {
						public void run() {
							try {
								// decide if the mutant is running due to
								// infinite loops
								mutantRunning = true;
								...
								for (int k = 0; k < testCases.length; k++) {
									...
									for (Annotation annotation : annotations) {
										...
										// annotation.toString() +
										// annotation.toString().indexOf("@org.junit.Test"));
										if (annotation.toString().indexOf(
												"@org.junit.Test") != -1) {
											mutantResults.put(
													testCases[k].getName(),
													"pass");
								...
								List<Failure> listOfFailure = result
										.getFailures();
								for (Failure failure : listOfFailure) {
									String nameOfTest = failure.getTestHeader()
											.substring(
													0,
													failure.getTestHeader()
															.indexOf("("));
									String testSourceName = testSet + "."
											+ nameOfTest;
								...
								synchronized (lockObject) {
									// release the lock when the execution of
									// tests on this mutant is finished
									lockObject.notify();
								}
					...
					synchronized (lockObject) {
						// the lock is released when the TIMEOUT is done or the
						// test execution is finished
						lockObject.wait(TIMEOUT);
					}
					if (mutantRunning) {
						// the test execution is interrupted if the execution
						// time execeeds the TIMEOUT
						t.interrupt();
						System.out.println(" time_out: more than " + TIMEOUT
								+ " seconds");
						// put the result in the mutant result when detecting an
						// infinite loop
						for (int k = 0; k < testCases.length; k++) {
							Annotation[] annotations = testCases[k]
									.getDeclaredAnnotations();
				...
				for (int k = 0; k < junitTests.size(); k++) {
					...
					if (!mutantResults.get(name).equals(
							originalResults.get(name))) {
						sign = true;
						// update the final results by tests
						if (finalTestResults.get(name).equals(""))
							finalTestResults.put(name, mutant_name);
						else
							finalTestResults.put(name,
									finalTestResults.get(name) + ", "
											+ mutant_name);
						...
						if (finalMutantResults.get(mutant_name).equals(""))
							...
							finalMutantResults.put(mutant_name,
									finalMutantResults.get(mutant_name) + ", "
											+ name);
					}
				}
				if (sign == true)
	 ...
	 * System.out.println("Deleting directories of killed mutants"); for(int
	 * i=0;i<v.size();i++){ System.out.print(v.get(i).toString()+" ");
	 * erase_directory(v.get(i).toString()); } }
	 * 
	 * void erase_directory(String mutant_name){ File mutant_dir = new
	 * File(MutationSystem.MUTANT_PATH+"/"+mutant_name); File[] f =
	 * mutant_dir.listFiles(); boolean flag = false; for(int
	 * i=0;i<f.length;i++){ while(!flag){ flag = f[i].delete(); } flag = false;
	 * }
	 * 
	 * while(!flag){ flag = mutant_dir.delete(); } }
	 */
}
