...
public class MutationSystem extends OJSystem {
	...
	public static String SYSTEM_HOME = "E:/Mutation";
	...
	public static String[] cm_operators = { "IHI", "IHD", "IOD", "IOP", "IOR",
			"ISI", "ISD", "IPC", // 8��
	...
	// public static String[] first_tm_operators = {
	// "ABS","AOR","LCR","ROR","UOI" };
	/*
	...
	// Upsorn: (04/06/2009) added Statement Deletion operator (SID, SWD, SFD,
	// SSD)
	public static String[] tm_operators = { "AORB", "AORS", "AOIU", "AOIS",
			"AODU", "AODS", "ROR", "COR", "COD", "COI", "SOR", "LOR", "LOI",
			"LOD", "ASRS" };
	// "IBD", "WBD", "FBD", "SBD" };
	// "SID", "SWD", "SFD", "SSD" };
	// "SDL"};
	...
	public static String[] em_operators = { "EFD", "EHC", "EHD", "EHI", "ETC",
			"ETD" };
	...
	protected static String getJavacArgForDir(String dir, String str,
		...
		if (javaF.length > 0) {
			...
			for (int k = 0; k < javaF.length; k++) {
				...
				temp.replace('\\', '/');
				targetFiles.add(temp.substring(
						MutationSystem.SRC_PATH.length() + 1, temp.length()));
			}
		}
		...
		for (int i = 0; i < sub_dir.length; i++) {
			result = getJavacArgForDir(sub_dir[i].getAbsolutePath(), result,
					targetFiles);
		}
		return result;
	}
	...
	public static void setJMutationStructure() {
		try {
			...
			String home_path = str.substring("MuJava_HOME=".length(),
					str.length());
