 ...
 * 
 *      Modified by Nan Li on 09/29/2012 evaluateDown(typeParameter) and
 *      record(Environment, String type, String name) are added if a method has
 *      type parameters in which identifiers extends from some types, the
 ...
 *      type parameter, T and its type Comparable should be recorded such that T
 *      could be recognized as a variable of Comparable in this method
 */
public class VariableBinder extends ScopeHandler {
	...
	public Statement evaluateDown(ForStatement ptree) throws ParseTreeException {
		...
		if (vdecls != null)
			...
			bindName(getEnvironment(),
					Toolbox.nameToJavaClassName(tspec.toString()), identifier);
		}
	...
	public TypeParameter evaluateDown(TypeParameter ptree)
			throws ParseTreeException {
		super.evaluateDown(ptree);
	...
	private static void bindForInit(TypeName tspec,
		...
		// If the for statement is a enhanced one, variable declarator will be
		// null
	...
	private static void record(Environment env, String type, String name) {
		...
		try {
			...
			System.err.println("VariableBinder.record() " + e.toString()
					+ " : " + qtypename);
