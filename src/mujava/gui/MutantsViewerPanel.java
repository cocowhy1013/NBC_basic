...
public abstract class MutantsViewerPanel extends JPanel {
	...
	void showGeneratedMutantsNum(String[] name) {
		...
		switch (getMutantType()) {
		case MutationSystem.CM:
			operators = MutationSystem.cm_operators; // 클래스 수준 뮤테이션 연산자
			break;
		case MutationSystem.TM:
			operators = MutationSystem.tm_operators; // 메소드 수준 뮤테이션 연산자
			break;
		case MutationSystem.EM:
			operators = MutationSystem.em_operators; // 예외상황 뮤테이션 연산자
			break;
	...
	void mList_mouseClicked(MouseEvent e) {
		...
		if (selected_obj != null) {
			...
			if (mutant_log != null) {
				System.out
						.println("MutantsViewerPanel.mList_mouseClicked - mutant_log != null");
	...
	public void showOriginal() {
		try {
			String strLine;
			File myFile = new File(MutationSystem.ORIGINAL_PATH,
	...
	public void showMutant(String dir_name, int changed_line) {
		...
		try {
			...
			int caret_pos = 0;
			String strLine;
			File myFile = new File(MutationSystem.MUTANT_PATH + "/" + dir_name,
					MutationSystem.CLASS_NAME + ".java");
		...
		try {
			...
			while ((strLine = lReader.readLine()) != null) {
				if (strLine.indexOf(name) == 0) {
					...
					changeTF.setText(" (line " + changed_line + ") "
							+ changed_content);
					changeTF.repaint();
					mutantTP.repaint();
					return changed_line;
	...
	int getMutatedLineNum(String str) {
		...
		// (Upsorn, 07/14/2009) Modify end_index. Originally, a line of log is
		// formatted as
		...
		// Here, to make the format flexible, consider start_index with the
		// first ":"
		// end_index with the second ":"
		String log_str = str;
		log_str = log_str.substring(start_index + 1);
		int end_index = start_index + 1
				+ log_str.indexOf(MutationSystem.LOG_IDENTIFIER);
