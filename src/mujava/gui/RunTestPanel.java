...
public class RunTestPanel extends JPanel implements ActionListener {
	...
	JRadioButton onlyClassButton = new JRadioButton(
			"Execute only class mutants");
	JRadioButton onlyTraditionalButton = new JRadioButton(
			"Execute only traditional mutants");
	JRadioButton onlyExceptionButton = new JRadioButton(
			"Execute only exception mutants");
	...
	void jbInit() {
		...
		traditional_summaryPanel.setLayout(new BoxLayout(
				traditional_summaryPanel, BoxLayout.PAGE_AXIS));
		...
		class_summaryPanel.setLayout(new BoxLayout(class_summaryPanel,
				BoxLayout.PAGE_AXIS));
	...
	String[] eraseExtension(String[] list, String extension) {
		...
		for (int i = 0; i < list.length; i++) {
			result[i] = list[i].substring(0,
					list[i].length() - extension.length() - 1);
		}
		return result;
	}
	...
	void getTestSetNames(File testDir, Vector v) {
		...
		if (start_index < end_index)
			start_index++;
		String suffix = testDir.getAbsolutePath().substring(start_index,
				end_index);
		...
		if ((targetClassObj != null) && (testSetObject != null)) {
			...
			try {
				if (onlyClassButton.isSelected()) {
					...
					showResult(test_result, cResultTable, cKilledList,
							cLiveList);
					...
					test_result = test_engine
							.runTraditionalMutants(methodSignature.toString());
					showResult(test_result, tResultTable, tKilledList,
							tLiveList);
			...
			System.out
					.println(" [Error] Please check test target or test suite ");
		}
	}

	private void showEmptyResult(JTable table, JList killed_list,
			JList live_list) {
	...
	private void showResult(TestResult tr, JTable table, JList killed_list,
			JList live_list) {
		int i;
		// Mutation Score
		if (tr == null)
		...
		resultModel.setValueAt("  " + (new Integer(live_num)).toString(), 0, 1); // live
																					// mutant
		resultModel.setValueAt("  " + (new Integer(killed_num)).toString(), 1,
				1); // killed mutant
		resultModel.setValueAt(
				"  " + (new Integer(live_num + killed_num)).toString(), 2, 1); // total
	...
	void changeContents() {
		...
		try {
			...
			System.err
					.println("error at updateClassComboBox() in RunTestPanel");
		}
		this.repaint();
	}
	...
	void showGeneratedClassMutantsNum(String[] name) {
		if (name != null) {
			...
			int total = 0;
			CMSummaryTableModel myModel = (CMSummaryTableModel) (cmTable
					.getModel());
	...
	void showGeneratedTraditionalMutantsNum(String[] name) {
		if (name != null) {
			...
			int total = 0;
			TMSummaryTableModel myModel = (TMSummaryTableModel) (tmTable
					.getModel());
			for (int i = 0; i < MutationSystem.tm_operators.length; i++) {
				myModel.setValueAt(new Integer(num[i]), i, 1);
				total = total + num[i];
