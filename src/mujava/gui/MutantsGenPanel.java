...
public class MutantsGenPanel extends JPanel {
	...
	void jbInit() {
		...
		FileTableModel fTableModel = new FileTableModel(
				MutationSystem.getNewTragetFiles());
		...
		traditional_operator_panel.setLayout(new BoxLayout(
				traditional_operator_panel, BoxLayout.PAGE_AXIS));
		...
		traditional_operator_scrollP.getViewport()
				.add(traditionalOpTable, null);
		...
		class_operator_panel.setLayout(new BoxLayout(class_operator_panel,
				BoxLayout.PAGE_AXIS));
	...
	void runB_mouseClicked(MouseEvent e) {
		...
		TMOTableModel tmoTableModel = (TMOTableModel) traditionalOpTable
				.getModel();
