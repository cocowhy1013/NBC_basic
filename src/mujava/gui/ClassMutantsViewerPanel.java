...
public class ClassMutantsViewerPanel extends MutantsViewerPanel {
	...
	void jbInit() throws Exception {
		...
		/**
		 * summary table: containing the numbers of mutants by each class-level
	...
	void printGeneratedMutantNum(String[] operators, int[] num) {
		 ...
		 * int[] num = new int[operators.length]; // 뮤테이션 연산자 별 개수값 저장 for(int
		 * i=0; i<operators.length; i++){ // num 배열 초기화 num[i] = 0; }
		 * if(name!=null){ for(int i=0;i<name.length;i++){ for(int j=0;
		 * j<operators.length;j++){ // ==>
		 * if(name[i].indexOf(operators[j]+"_")==0){ num[j]++;} // ==> } } }
		 */
		// 화면에 보여주는 소스
		int total = 0;
		CMSummaryTableModel myModel = (CMSummaryTableModel) (summaryTable
				.getModel());
