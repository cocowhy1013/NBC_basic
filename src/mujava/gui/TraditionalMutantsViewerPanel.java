...
public class TraditionalMutantsViewerPanel extends MutantsViewerPanel {
	...
	void jbInit() throws Exception {
		...
		JLabel selectMethodLabel = new JLabel(
				"                                   Select a method : ");
		...
		/**
		 * show a list of mutants to be selected for viewing -- click on the
		 * mutant to display the original source and mutated code
		 */
	...
	void updateContents() {
		try {
			...
			System.err
					.println("Error in update() in TraditionalMutantsViewerPanel.java");
		}
	}
	...
	void updateClassComboBox() {
		...
		if (isProperClass(target_dir)) {
			...
			try {
				...
				System.err
						.println("error at updateClassComboBox() in TraditionalMutantsViewerPanel");
			}
		} else {
			clearSourceContents();
	...
	void printGeneratedMutantNum(String[] operators, int[] num) {
		int total = 0;
		TMSummaryTableModel myModel = (TMSummaryTableModel) (summaryTable
				.getModel());
